...
public class LFunctionHubTask extends LTask {
    public static LFunctionHubTask getActiveTask(){
        if(LynxMindEndTickEventManager.isTaskActive(LTaskType.FUNCTION_HUB)){
            var task = LynxMindEndTickEventManager.getTask(LTaskType.FUNCTION_HUB);
            if(!(task instanceof LFunctionHubTask lTask)) return null;
            return lTask;
        }
        else{
            var registered = new LFunctionHubTask();
