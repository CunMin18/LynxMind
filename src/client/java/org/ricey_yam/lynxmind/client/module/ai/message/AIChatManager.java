package org.ricey_yam.lynxmind.client.module.ai.message;
...
import org.ricey_yam.lynxmind.client.module.ai.message.event.player.sub.PlayerCreateTaskEvent;
import org.ricey_yam.lynxmind.client.module.ai.message.event.player.sub.PlayerRemoveTaskEvent;
...
public class AIChatManager {
    ...
    public static void handleAIReply(String reply){
        /// 这里会对AI信息进行一次过滤 过滤工艺后期会不断优化
        /// 我能做到的就这些了 如果AI再发癫 建议换模型
        reply = AIJsonHandler.filterOutJson(reply);
        /// 格式正确
        if(StringUtils.isJson(reply)){
            wrongReplyFormatTriedCount = 0;
            var deserialized = AIJsonHandler.deserialize(reply);
    ...
    public static CompletableFuture<String> sendStartMessageToAIAndReceiveReply(){
        ...
        return AIServiceManager.sendMessageAndReceiveReplyAsync(UserMessageStorage.startMessage);
    }

    /// 发送帮助信息给AI
    public static CompletableFuture<String> sendHelpMessageToAIAndReceiveReply(){
        System.out.println("正在重新介绍游戏规则给AI...");
        return AIServiceManager.sendMessageAndReceiveReplyAsync(UserMessageStorage.helpMessage);
    }
    ...
    public static CompletableFuture<String> sendTaskMessageToAIAndReceiveReply(){
        ...
        var serialized = AIJsonHandler.serialize(playerCreateTaskEvent);
        return AIServiceManager.sendMessageAndReceiveReplyAsync(serialized);
    }
    ...
    public static CompletableFuture<String> sendRemovingTaskMessageToAIAndReceiveReply(){
        ...
        var serialized = AIJsonHandler.serialize(playerRemoveTaskEvent);
        return AIServiceManager.sendMessageAndReceiveReplyAsync(serialized);
    }
